...
export default function DrawerContent(props) {
  ...
  const [switchAccountSheetVisible, setSwitchAccountSheetVisible] = useState(
    false
  );
  const [qrCodeVisible, setQrCodeVisible] = useState(false);
  const [tutorialVisible, setTutorialVisible] = useState(false);
  const toggleSwitchAccountSheet = () =>
    setSwitchAccountSheetVisible(!switchAccountSheetVisible);
  ...
  const QrCodeItem = (
    <QrCode key="qrCode" onPress={() => setQrCodeVisible(!qrCodeVisible)} />
  );
  const SwitchAccountItem = (
    <SwitchAccount key="switchAccount" onPress={toggleSwitchAccountSheet} />
  );
  const LogOutItem = <LogOut key="logOut" onPress={onLogout} />;

  // insert the drawer items into the list
  items.splice(1, 0, QrCodeItem);
  items.splice(3, 0, TutorialItem);
  items.splice(4, 0, SwitchAccountItem);
  items.splice(5, 0, LogOutItem);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Popup
        ...
        popupContent={
          <YoutubePlayer
            videoId={language == "EN" ? "gEUEJDUNwOk" : "ErtrATOfWVQ"}
            height={200}
            width={280}
          />
        }
      ...
      <Popup
        ...
        popupContent={
          <View style={({ width: "100%" }, { alignItems: "center" })}>
            <Text>{(currentUser.uid, console.log(currentUser.uid))}</Text>
            <QRCode value={currentUser.uid} size={150} />
          </View>
        }
      ...
      <BottomSheet
        visible={switchAccountSheetVisible}
        title={I18n.t("switchAccount")}
        sheetContent={
          <SwitchAccountSheetContent
            navigation={props.navigation}
            dismiss={toggleSwitchAccountSheet}
          />
        }
        onDismiss={toggleSwitchAccountSheet}
      />
    </SafeAreaView>
  );
}
